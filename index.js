...
const user = createPaddle(0, canvas.height / 2 - paddleHeight / 2, paddleWidth, paddleHeight, "#00CCFF");

const com = createPaddle(canvas.width - paddleWidth, canvas.height / 2 - paddleHeight / 2, paddleWidth, paddleHeight, "#000066");

// Define ball object
const ball = createBall(canvas.width / 2, canvas.height / 2, ballRadius, initialBallSpeed, initialBallSpeed, "RED");
...
function render(){
    ...
    drawRect(0, 0, canvas.width, canvas.height, "#0066CC");
    drawNet();
    
    // Draw scores
    drawText(user.score, canvas.width / 4, canvas.height / 2, "AZURE", 120, 'bold');
    drawText(com.score, (3 * canvas.width) / 4, canvas.height / 2, "AZURE", 120, 'bold');
...
// Set game Loop to run at 60 frame per second
